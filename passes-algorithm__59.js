...
function scoreAttraction(attraction, userPreferences) {
  ...
  userPreferences.interests.forEach(interest => {
    ...
    if (attraction.category_secondary.includes(categoryValue)) {
      score += 20;
      scoreEl += `<div><b><i>Interests Category Secondary:</i></b> <u>${attraction.category}</u> <b><i>user_preference:</i></b> <u>${categoryValue}</u> (+20 points) current_score: <b>${score}</b></div>`;
    }
