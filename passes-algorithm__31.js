...
// let groupSize = Number(document.querySelector('.ak-per-day').value) || attractionsToShowPerDay;
// let maxRecommendationsCount = Number(document.querySelector('.ak-max-recommends').value) || userMaxRecommendationsCount;

$processLead.addEventListener('click', async e => {
  e.preventDefault();

  const groupSize = Number(document.querySelector('.ak-per-day').value) || attractionsToShowPerDay;
  const maxRecommendationsCount = Number(document.querySelector('.ak-max-recommends').value) || userMaxRecommendationsCount;
...
function getRecommendations(userPreferences, allAttractions, groupSize, maxRecommendationsCount) {
  ...
  console.log('tripLength * 2:', tripLength * 2)
  console.log('maxRecommendations (2 per day, max 10):', maxRecommendations)
