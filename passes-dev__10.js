...
$pace.addEventListener('change', e => {
  if ($pace.value.includes('relaxed')) {
    groupSize = 2;
  }
  else if ($pace.value.includes('moderate')) {
    groupSize = 3;
  }
  else if ($pace.value.includes('packed')) {
    groupSize = 4;
  }
});
...
$processPass.addEventListener('click', async e => {
  ...
  const filteredAttractions = getRecommendations(userPreferences, Attractions, groupSize, maxRecommendationsCount); 
  ...
  filteredAttractions.forEach(attraction => {
    ...
    const $desc = $result.querySelector('[data-ak="desc"]');
    const $neighborhood = $result.querySelector('[data-ak="neighborhood"]');
    const $cost = $result.querySelector('[data-ak="cost"]');
    const $individualPrice = $result.querySelector('[data-ak="individual-price"]');
    const $passes = $result.querySelector('[data-ak="passes"]');
    const $bestFor = $result.querySelector('[data-ak="best-for"]');
    const $category = $result.querySelector('[data-ak="category"]');
    const $categorySec = $result.querySelector('[data-ak="category-sec"]');
    ...
    const { on_pass, 
      ...
      best_for, 
      category, 
      category_secondary } = attraction;
    if (on_pass.trim().toLowerCase() === 'true') {
      $onPass.classList.remove('hide');
    }
    else {
      $offPass.classList.remove('hide');
    }
    ...
    $desc.textContent = why_description;
    $neighborhood.textContent = neighborhood;
    $cost.textContent = cost;
    $individualPrice.textContent = individual_price;
    $passes.textContent = passes;
    $bestFor.textContent = best_for;
    $category.textContent = category;
    $categorySec.textContent = category_secondary;
