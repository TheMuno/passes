...
$processLead.addEventListener('click', async e => {
  ...
  if (filteredAttractions.length) {
    // 1. Group the filtered results by neighborhood
    const groupedByNeighborhood = filteredAttractions.reduce((acc, attraction) => {
      const area = attraction.neighborhood || "Other Areas";
      if (!acc[area]) acc[area] = [];
      acc[area].push(attraction);
      return acc;
    }, {});

    let dayCount = 1;

    // 2. Loop through each neighborhood group
    Object.keys(groupedByNeighborhood).forEach((neighborhoodName) => {
      const attractionsInArea = groupedByNeighborhood[neighborhoodName];
      
      // 3. Split the attractions in this neighborhood into "Pace-sized" chunks
      // This ensures if groupSize is 3, but there are 5 items, it creates 2 days for that area.
      for (let i = 0; i < attractionsInArea.length; i += groupSize) {
        const dailyChunk = attractionsInArea.slice(i, i + groupSize);

        // Create Header for the Day
        const $groupHeader = document.createElement('div');
        $groupHeader.className = 'ak-attraction-result-day'; 
        $groupHeader.textContent = `Day ${dayCount}: ${neighborhoodName}`;
        $resultsWrap.append($groupHeader);

        // 4. Render the attractions for this specific chunk/day
        dailyChunk.forEach((attraction) => {
          const { 
            attraction_name, 
            why_description, 
            on_pass, 
          ...
          $div.innerHTML = `
            <div><b>${attraction_name}</b> - ${why_description}</div>
            <br>
            ...
            <div><b>Score Spread:</b> ${scoreEl}</div>
            <br><br>`;
          $resultsWrap.append($div);
        });

        // Increment day counter after each chunk is rendered
        dayCount++;
      }
    });
  } else {
  ...
  if (filteredAttractions.length) {
    ...
    Object.keys(groupedByNeighborhood).forEach((neighborhoodName) => {
      ...
      // Create a Day + Neighborhood Header
      const $groupHeader = document.createElement('div');
      $groupHeader.className = 'ak-attraction-result-day'; 
      // This combines "Day X" with the Neighborhood name
      $groupHeader.textContent = `Day ${dayCount}: ${neighborhoodName}`;
      $resultsWrap.append($groupHeader);

      // 3. Loop through attractions within THIS neighborhood
      groupedByNeighborhood[neighborhoodName].forEach((attraction) => {
        const { 
          attraction_name, 
          why_description, 
          on_pass, 
      ...
      // Increment day for the next neighborhood group
      dayCount++;
    });
  } else {
    $resultsWrap.innerHTML = '<i>No attractions match!</i>';
  ...
  if (filteredAttractions.length) {
    ...
    Object.keys(groupedByNeighborhood).forEach((neighborhoodName) => {
      ...
      // Create a Neighborhood Header
      const $groupHeader = document.createElement('div');
      $groupHeader.className = 'ak-attraction-result-day'; // Reusing your "Day" styling
      // $groupHeader.style.backgroundColor = '#333'; // Optional: slight color change to distinguish from "Days"
      $groupHeader.textContent = `Neighborhood: ${neighborhoodName}`;
      $resultsWrap.append($groupHeader);
...
function scoreAttraction(attraction, userPreferences) {
  ...
  if (bestFor.includes(userPreferences.travelGroup) || bestFor.includes('all')) {
    ...
    score += 10;
    scoreEl += `<div><b><i>Best_for:</i></b> <u>${bestFor}</u> <b><i>user_preference:</i></b> <u>${userPreferences.travelGroup}</u> (+10 points) current_score: <b>${score}</b></div>`;
  }
  else {
    scoreEl += `<div><b><i>Best_for:</i></b> <u>${bestFor}</u> <b><i>user_preference:</i></b> <u>${userPreferences.travelGroup}</u> (+10 points) current_score: <b>${score}</b></div>`;
  }
  ...
  if (paceOptions.includes(userPreferences.pace_friendly)) {
    score += 5;
    scoreEl += `<div><b><i>Pace:</i></b> <u>${paceOptions}</u> <b><i>user_preference:</i></b> <u>${userPreferences.pace_friendly}</u> (+5 points) current_score: <b>${score}</b></div>`;
  }
