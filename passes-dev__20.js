...
$processPass.addEventListener('click', async e => {
  ...
  const cityPassMatches = passData.filter(([_, data]) => data.pass_id.includes('citypass'));
  const bestCityPass = cityPassMatches
    .sort((a, b) => Number(a.attraction_count) - Number(b.attraction_count))
    .find(pass => pass.attraction_count >= attractionsNum);
