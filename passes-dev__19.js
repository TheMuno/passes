...
$processPass.addEventListener('click', async e => {
  ...
  const cityPassMatches = passData.filter(([pass, data]) => data.pass_id.includes('citypass'));
  const goCityMatches = passData.filter(([pass, data]) => data.pass_id.includes('gocity_explorer'));
