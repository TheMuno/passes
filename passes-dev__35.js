...
$processPass.addEventListener('click', async e => {
  ...
  localStorage.removeItem('ak-go-city-exact'); // get rid of exact incase no longer needed
  localStorage.removeItem('ak-city-pass-exact'); 
  ...
  function populatePasses(passData, passName, savedPassName) {
    ...
    if (exactPass) {
      ...
      localStorage[`ak-${savedPassName}-name`] = exactPass[0];
      localStorage[`ak-${savedPassName}-price`] = exactPass[1].pass_price;
      localStorage[`ak-${savedPassName}-exact`] = true; 
    }
    else {
      ...
      localStorage[`ak-${savedPassName}-lower-name`] = passLowerLimit[0];
      localStorage[`ak-${savedPassName}-lower-price`] = passLowerLimit[1].pass_price;
      localStorage[`ak-${savedPassName}-upper-name`] = passUpperLimit[0];
      localStorage[`ak-${savedPassName}-upper-price`] = passUpperLimit[1].pass_price;
    }
  }
