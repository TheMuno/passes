...
$processPass.addEventListener('click', async e => {
  ...
  Object.keys(localStorage).forEach(key => { // get rid of earlier saved values 
    if (key.startsWith('ak-pass')) {
      localStorage.removeItem(key);
    }
  });  
 
  populatePasses(passData, 'citypass', 'city-pass'); 
  populatePasses(passData, 'gocity_explorer', 'go-city'); 

  function populatePasses(passData, passName, savedPassName) {
    const matches = passData.filter(([id, data]) => data.pass_id.includes(passName));
    const sortedPasses = matches
      .sort((a, b) => Number(a[1].attraction_count) - Number(b[1].attraction_count));

    // let bestCityPass;
    // console.log('attractionsNum:', attractionsNum)  
    const exactPass = sortedPasses.find(([id, pass]) => {
      // console.log('pass.attraction_count:', pass.attraction_count)
      return Number(pass.attraction_count) === attractionsNum
    });
    ...
    if (exactPass) {
      ...
      // bestCityPass = `${exactPass[0]} - ${exactPass[1].pass_price}`;
      localStorage[`ak-pass-${savedPassName}-name`] = exactPass[1].pass_name;
      localStorage[`ak-pass-${savedPassName}-price`] = exactPass[1].pass_price;
      localStorage[`ak-pass-${savedPassName}-exact`] = true; 
    }
    else {
      const passUpperLimit = sortedPasses.find(([id, pass]) => pass.attraction_count >= attractionsNum);
      const passLowerLimit = sortedPasses.reverse().find(([id, pass]) => pass.attraction_count <= attractionsNum);

      // bestCityPass = `${passLowerLimit[0]} - ${passLowerLimit[1].pass_price} ${passUpperLimit[0]} - ${passUpperLimit[1].pass_price}`;
      localStorage[`ak-pass-${savedPassName}-lower-name`] = passLowerLimit[1].pass_name;
      localStorage[`ak-pass-${savedPassName}-lower-price`] = passLowerLimit[1].pass_price;
      localStorage[`ak-pass-${savedPassName}-upper-name`] = passUpperLimit[1].pass_name;
      localStorage[`ak-pass-${savedPassName}-upper-price`] = passUpperLimit[1].pass_price;
    }
  }
