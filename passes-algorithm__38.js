...
const relaxedRecommendationsCount = 2;
const moderateRecommendationsCount = 3;
const packedRecommendationsCount = 4;
...
$processLead.addEventListener('click', async e => {
  ...
  const groupSize = Number($perDay.value) || attractionsToShowPerDay;
  const maxRecommendationsCount = Number(localStorage['ak-maxRecommendationsCount']) || Number($maxRecommends.value) || userMaxRecommendationsCount;
...
$paceRadioBtns.forEach(paceRadio => {
    ...
    if (paceValue.includes('relaxed')) {
      localStorage['ak-maxRecommendationsCount'] = relaxedRecommendationsCount;
      $perDay.value = relaxedRecommendationsCount;
    }
    else if (paceValue.includes('moderate')) {
      localStorage['ak-maxRecommendationsCount'] = moderateRecommendationsCount;
      $perDay.value = moderateRecommendationsCount;
    }
    else if (paceValue.includes('packed')) {
      localStorage['ak-maxRecommendationsCount'] = packedRecommendationsCount;
      $perDay.value = packedRecommendationsCount;
    }
	});
});
