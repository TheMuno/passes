...
$processPass.addEventListener('click', async e => {
  ...
  function populatePasses(passData, passName, savedPassName) {
    ...
    const exactPass = sortedPasses.find(([id, pass]) => pass.attraction_count === attractionsNum);
    ...
    if (exactPass) {
      ...
      localStorage[`ak-pass-${savedPassName}-name`] = exactPass[0];
      ...
      localStorage[`ak-pass-${savedPassName}-lower-name`] = passLowerLimit[0];
      localStorage[`ak-pass-${savedPassName}-lower-price`] = passLowerLimit[1].pass_price;
      localStorage[`ak-pass-${savedPassName}-upper-name`] = passUpperLimit[0];
