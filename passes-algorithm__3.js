...
$processLead.addEventListener('click', async e => {
  ...
  getRecommendations(userPreferences, allAttractions); 
});
...
$dropDownLinks.forEach(link => {
  link.addEventListener('click', e => {
    ...
    const dropdownType = link.closest('[data-ak-dropdown]').querySelector('.ak-dropdown-toggle-text').textContent;
...
function filterByAvailability(attractions, userTravelDates) {
  return attractions.filter(attraction => {
    // Check if active
    if (attraction.active !== true) return false;
    
    // HOLIDAYS_ONLY CHECK (Updated column name)
    if (attraction.holidays_only === true) {
