...
$processPass.addEventListener('click', async e => {
  ...
  if (cityPassExact) {
    bestCityPass = cityPassExact[0];
  }
  else {
    const cityPassUpperLimit = cityPassSorted.find(([id, pass]) => pass.attraction_count >= attractionsNum)[0];
    const cityPassLowerLimit = cityPassSorted.reverse().find(([id, pass]) => pass.attraction_count <= attractionsNum)[0];

    bestCityPass = `${cityPassLowerLimit} ${cityPassUpperLimit}`;
  }
